...
# Page config
st.set_page_config("Random Forest Classification with Diabetes Dataset", layout="wide")
...
st.markdown(
    """
<div class="card">
<h1>🏥 Random Forest Classification: Diabetes Prediction</h1>
<p>Predict whether a person has diabetes using <b>Random Forest Classification</b> with optimized hyperparameters</p>
</div>
""",
...
with col1:
    st.metric("Estimators", hyperparams_dict['best_params'].get('n_estimators', '-'))
with col2:
    st.metric("Max Depth", hyperparams_dict['best_params'].get('max_depth', '-'))
with col3:
    st.metric("Min Samples Split", hyperparams_dict['best_params'].get('min_samples_split', '-'))
st.markdown("</div>", unsafe_allow_html=True)
